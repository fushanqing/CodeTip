...
public interface Creator {
     ...
     * sourceCode = '类的内部}'
     */
    static Pair<Integer, String> createClassOrInterface(RangeString sourceCode, Creator[] creators, JavaDocs javaDocs, AnnotationUsingMembers annotationUsingMembers, JavaSourceNode fatherNode, EnumFieldMembers enumFieldMembers) {
    ...
    static Pair<Integer, String> defualt(RangeString sourceCode, JavaDocs javaDocs, AnnotationUsingMembers annotationUsingMembers, JavaSourceNode fatherNode, MatcherBuilder<? extends JavaSourceNode> matcherBuilder, List list) {
        if (sourceCode.isEmpty()) {
            return Pair.of(-1, sourceCode.s);
        }
        Pair<? extends JavaSourceNode, String> build = matcherBuilder.build(sourceCode, javaDocs, annotationUsingMembers, fatherNode);
        if (build != null) {
     ...
     */
    Pair<Integer, String> create(RangeString sourceCode, JavaDocs javaDocs, AnnotationUsingMembers annotationUsingMembers, JavaSourceNode fatherNode);

    default List<? extends JavaSourceNode> getChildNodeMember() {
        return null;
    }
}
